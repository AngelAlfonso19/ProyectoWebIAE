...
function assignmentToHtml(obj, objiae) {
    console.log('bj name',obj.SubjectName);
    if (obj != undefined)
        return `
        <!-- Row1 -->
        <tr >
          ...
          <td> 
            ...
            <p id = "${objiae.pollID}" onclick="deleteIAE(this.id)">X</p>
              <div class="row">
                <div class="col-12 mt-3 d-flex justify-content-center">
                  <p id ="${objiae.pollID}" onclick="openassesment(this.id)" class="userName">${obj.subjectName}</p>
                </div>
              </div>
            </div>                                
          </td>
...
function getAssignment(data){
  var text = "";
   data.forEach(element => {
      ...
      BaseDatos3 = document.getElementById('1234A')
      mecAn = document.getElementById('65788')
